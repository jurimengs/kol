...
public class HikaricpConnection implements Connection {
	...
	@Override
	public String getId() {
		return CommonConstant.DB_HIKARICP;
	}
	...
	public HikaricpConnection(String driverClassName, String jdbcUrl, String username, String password,
			String connectionTestQuery, String connectionTimeout, String idleTimeout, String maxLifetime,
			String maximumPoolSize, String minimumIdle) throws BusinessException {
		...
		dbConfig.setConnectionTestQuery(connectionTestQuery);
		dbConfig.setConnectionTimeout(Long.valueOf(connectionTimeout));
		dbConfig.setIdleTimeout(Long.valueOf(idleTimeout));
		dbConfig.setMaxLifetime(Long.valueOf(maxLifetime));
		dbConfig.setMaximumPoolSize(Integer.valueOf(maximumPoolSize));
		dbConfig.setMinimumIdle(Integer.valueOf(minimumIdle));
		
		HikariDataSource temp = new HikariDataSource(dbConfig);
		this.template = temp;
	}

	public HikaricpConnection(Properties pro) {
		HikariConfig dbConfig = new HikariConfig(pro);
		HikariDataSource temp = new HikariDataSource(dbConfig);
		this.template = temp;
	}
	
	public HikaricpConnection() throws BusinessException {
		InputStream pin = null;
		
		Properties pro = new Properties();
		String fileName = "/WEB-INF/config/directdb_oracle_db.properties";
		try {
